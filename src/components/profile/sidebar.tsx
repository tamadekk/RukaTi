const Sidebar = ({ user }: { user: { name: string; avatar: string } }) => (
  <aside className="w-full md:w-64 bg-white border-r flex flex-row md:flex-col justify-between py-4 md:py-6 px-2 md:px-4 h-20 md:h-auto fixed md:static bottom-0 left-0 right-0 z-20 md:z-auto">
    ...
    <button className="flex md:hidden items-center justify-center w-full py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm">
...
const SidebarLink = ({
  ...
  <a
    ...
    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium ${
      active ? "bg-green-900 text-white" : "text-gray-700 hover:bg-gray-100"
    }`}
  >
    {label}
  </a>
