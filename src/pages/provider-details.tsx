...
export function ProviderDetailsPage() {
  ...
  const { providerProfile, providerServices, loading, error, fetchProviderDetails } = useMarketStore();
      ...
      <div className="space-y-8 font-mono max-w-6xl mx-auto w-full">
         ...
         <div className="border border-black bg-white p-8 flex flex-col md:flex-row gap-8 items-start md:items-center">
             <Avatar className="w-32 h-32 border-2 border-black rounded-none">
                <AvatarImage src={providerProfile.avatar || undefined} className="object-cover" />
             ...
             <div className="flex-1 space-y-4">
                 <div>
                    <h1 className="text-3xl font-bold uppercase tracking-tight">{providerProfile.email?.split('@')[0]}</h1>
                    <div className="flex items-center gap-4 text-sm mt-1">
                        <div className="flex items-center gap-1 text-yellow-500">
                             <Star className="w-4 h-4 fill-current" />
                             <span className="text-black font-bold">{providerProfile.rating || "New"}</span>
                        </div>
                        <div className="flex items-center gap-1 text-gray-500">
                             <MapPin className="w-4 h-4" />
                             <span>Joined {new Date(providerProfile.created_at || Date.now()).toLocaleDateString()}</span>
                        </div>
                    </div>
                 </div>
         ...
         <div className="space-y-4">
             <h2 className="text-xl font-bold uppercase border-b border-black pb-2">Active Listings ({providerServices.length})</h2>
             ...
             {providerServices.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {providerServices.map((service) => (
                        <ServiceCard key={service.service_id} service={service} readonly={true} />
                    ))}
                </div>
             )}
         </div>
         ...
         <div className="space-y-4 opacity-75">
            <h2 className="text-xl font-bold uppercase border-b border-black pb-2">Latest Reviews</h2>
